...
int searchCircularArray(int nums[], int n, int target)// Function to find an element `target` in a circularly sorted array
{
  int low = 0, high = n - 1;// search space is nums[low...high]
   while (low <= high)
    // loop till the search space is exhausted
    // find the mid-value in the search space and
    // compares it with the target
   int mid = (low + high)/2;// if the target is found, return its index
   if (target == nums[mid]) {
   return mid;
   }
